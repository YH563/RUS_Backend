...
namespace TestWrenchEstimate {
    ...
    void TestWrenchEstimatorSmoke(const std::string& urdf_path) {
        ...
        try {
            ...
            VectorXd qd       = VectorXd::Zero(6);
            VectorXd qdd      = VectorXd::Zero(6);
            VectorXd tau_meas = VectorXd::Zero(6);

            // 3. 原始估算接口
            const RusSimForce::ForceResult raw = est.Estimate(q, qd, qdd, tau_meas);
            RCLCPP_INFO(logger, "[PASS] Estimate() 调用成功");
            RCLCPP_INFO(logger, "       force  = [%s]", vec_to_str(raw.force).c_str());
            RCLCPP_INFO(logger, "       torque = [%s]", vec_to_str(raw.torque).c_str());
            RCLCPP_INFO(logger, "       cond   = %.4f, valid = %s",
                raw.cond_num, raw.is_valid ? "true" : "false");

            // 4. 法兰接口（真实输入模式）
            const RusSimForce::EndEffectorWrench flange_res =
                est.EstimateAtFlange(q, qd, qdd, tau_meas);
            RCLCPP_INFO(logger, "[PASS] EstimateAtFlange(q,qd,qdd,tau) 调用成功");
            RCLCPP_INFO(logger, "       p_flange       = [%s]", vec_to_str(flange_res.p_flange).c_str());
            RCLCPP_INFO(logger, "       force(base)    = [%s]", vec_to_str(flange_res.force).c_str());
            RCLCPP_INFO(logger, "       torque(base)   = [%s]", vec_to_str(flange_res.torque).c_str());
            RCLCPP_INFO(logger, "       force(flange)  = [%s]", vec_to_str(flange_res.force_in_flange).c_str());
            RCLCPP_INFO(logger, "       torque(flange) = [%s]", vec_to_str(flange_res.torque_in_flange).c_str());

            // 5. 仿真注入力接口
            VectorXd wrench_6d(6);
            ...
            RCLCPP_INFO(logger, "       sim.force(base)    = [%s]", vec_to_str(sim_res.force).c_str());
            RCLCPP_INFO(logger, "       sim.torque(base)   = [%s]", vec_to_str(sim_res.torque).c_str());
            RCLCPP_INFO(logger, "       sim.force(flange)  = [%s]", vec_to_str(sim_res.force_in_flange).c_str());
            RCLCPP_INFO(logger, "       sim.torque(flange) = [%s]", vec_to_str(sim_res.torque_in_flange).c_str());
